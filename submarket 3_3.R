...
summary(lm(V3*100~pct_black+pct_renter_occupied+pct_collegeDegree+
             MATHMeanScaleScore15+log_median_income, data=datanew))

datanew$prob <- colnames(datanew[, c(28:30)])[apply(datanew[, c(28:30)],1,which.max)]
...
datanew$submarket <- ifelse(datanew$prob == 1, "Submarket 1", ifelse(datanew$prob == 2, "Submarket 2", "Submarket 3"))
...
data1$tax_pred_sales <- ifelse(data1$quantile == 8, data1$tax_pred_sales+abs(rnorm(1, mean=0.003964, sd=0.0005)), 
                               ifelse(data1$quantile == 9, data1$tax_pred_sales+abs(rnorm(1, mean=0.119931, sd=0.005)), 
                                      ifelse(data1$quantile == 10, data1$tax_pred_sales-abs(rnorm(1, mean=0.004042, sd=0.010)), 
                                             ifelse(data1$quantile == 7, data1$tax_pred_sales+abs(rnorm(1, mean=0.0050842, sd=0.005)),
