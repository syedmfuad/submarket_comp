...
summary(lm(V4*100~pct_black+pct_renter_occupied+pct_collegeDegree+
             MATHMeanScaleScore15+log_median_income, data=datanew))

datanew$prob <- colnames(datanew[, c(28:31)])[apply(datanew[, c(28:31)],1,which.max)]
...
datanew$submarket <- ifelse(datanew$prob == 1, "Submarket 1", ifelse(datanew$prob == 2, "Submarket 2", ifelse(datanew$prob == 3, "Submarket 3", "Submarket 4")))

map1 +
  geom_point(data = datanew, aes(x = -lon, y = lat, colour = as.factor(submarket)), size = 1.10, alpha=0.75) +
